...
#define PADDING (self.frame.size.height/10.0)
#define kDefaultFont [UIFont systemFontOfSize:12.0]
...
NS_INLINE MPGraphValuesRange MPGraphValuesRangeMake(CGFloat min, CGFloat max) {
...
//定义Y轴表示结构体
struct _MPValuesRangeY{
    MPGraphValuesRange graphRange;
    CGFloat gap;
};
typedef struct _MPValuesRangeY MPGraphValuesRangeY;
//定义构造函数
NS_INLINE MPGraphValuesRangeY MPGraphValuesRangeYMake(MPGraphValuesRange graphRange,CGFloat gap){

    MPGraphValuesRangeY graphRangeY;
    graphRangeY.graphRange = graphRange;
    graphRangeY.gap = gap;
    return graphRangeY;
};
...
@property (nonatomic,copy) NSArray *values; // array of NSNumber or NSString or NSArray
