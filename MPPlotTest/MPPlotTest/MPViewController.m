...
- (void)viewDidLoad
{
    ...
    graph=[MPPlot plotWithType:MPPlotTypeGraph frame:CGRectMake(0, 30, 800, 150)];
    //graph.backgroundColor = [UIColor greenColor];
...
//
    ...
    graph2.lineColors=@[[UIColor redColor]];
    graph2.curved=YES;
    [self.view addSubview:graph2];
    
    
    graph3=[[MPGraphView alloc] initWithFrame:CGRectMake(0, 200, 800, 150)];
    ...
    graph3.lineColors=@[[UIColor colorWithRed:0.500 green:0.158 blue:1.000 alpha:1.000]];
    ...
    graph4.lineColors=@[[UIColor redColor]];
    graph4.curved=YES;
    ...
    graph5=[MPPlot plotWithType:MPPlotTypeBars frame:CGRectMake(0, 360, 800, 150)];
    graph5.waitToUpdate=YES;
    graph5.detailView=(UIView <MPDetailView> *)[self customDetailView];
    [graph5 setAlgorithm:^CGFloat(CGFloat x) {
        return tan(x);
    } numberOfPoints:8];
    graph5.lineColors=@[[UIColor colorWithRed:0.120 green:0.806 blue:0.157 alpha:1.000]];
