...
- (void)setState:(WSDPullRefreshState)aState{
	...
	switch (aState) {
		...
		case WSDPullRefreshNormal:
			//当前状态为释放更新 (释放->下拉)说明数据加载完成，需要复原箭头图片
...
- (void)egoRefreshScrollViewDidScroll:(UIScrollView *)scrollView {
	...
	if (_state == WSDPullRefreshLoading) {
		...
		if (_state == WSDPullRefreshPulling && scrollView.contentOffset.y > -65.0f && scrollView.contentOffset.y < 0.0f && !_loading) {
			[self setState:WSDPullRefreshNormal];
            //如果当前状态为下拉刷新，并且当前偏移量大于65，并且数据并不在加载，设置状态为释放刷新
		} else if (_state == WSDPullRefreshNormal && scrollView.contentOffset.y < -65.0f && !_loading) {
