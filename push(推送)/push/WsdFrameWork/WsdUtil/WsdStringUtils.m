...
+ (NSArray *)listIdsByArray:(NSArray *)arrays{
    NSMutableArray *results = [[NSMutableArray alloc] initWithCapacity:[arrays count]];
    static NSString* ID = @"id";
    id obj = [arrays lastObject];
    if([obj respondsToSelector:NSSelectorFromString(ID)]){
        for (int i = 0 ; i < [arrays count]; i++) {
            NSString *value = [arrays[i] valueForKey:ID];
...
+ (NSArray *)listIdsBySet:(NSSet *)set{
    NSMutableArray *results = [[NSMutableArray alloc] initWithCapacity:[set count]];
    static NSString* ID = @"id";
    id obj = [set anyObject];
    if([obj respondsToSelector:NSSelectorFromString(ID)]){
        for (id obj in set) {
            NSString *value = [obj valueForKey:ID];
            if ([self isNotBlank:value]) {
                [results addObject:value];
            }
        }
...
+ (BOOL)isEqualString1:(NSString *)str1 WithString2:(NSString *)str2{
    if ([self isNotBlank:str1]&&[self isNotBlank:str2]&&[str1 isEqualToString:str2]) {
        return YES;
    }else{
        return NO;
    }
