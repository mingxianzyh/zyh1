...
+(NSString*)dicSoapMessage:(NSString*)dicKey{
    NSMutableString *soap=[NSMutableString stringWithFormat:@"<%@>",dicKey];
    [soap appendString:@"%@"];
    [soap appendFormat:@"</%@>",dicKey];
    return soap;
}
...
+(NSString*)arrayToDefaultSoapMessage:(NSArray*)arr methodName:(NSString*)methodName{
    ...
    for (NSDictionary *item in arr) {
        ...
        [msg appendFormat:@"<%@ xmlns=\"\">",key];
        NSObject *obj = [item objectForKey:key];
        if([obj isKindOfClass:[NSDictionary class]]==YES){
            [msg appendString:[self dicAllSoapMessage:[item objectForKey:key] dicKey:key ]];
        }
        else{
...
+(NSString*)dicAllSoapMessage:(NSDictionary*)dic dicKey:(NSString*)dicKey{
    ...
    for(NSString *key in [dic allKeys]){
        ...
        if([obj isKindOfClass:[NSDictionary class]]==YES){
            [msg appendString:[self dicSoapMessage:dicKey ]];
        }
        else{
