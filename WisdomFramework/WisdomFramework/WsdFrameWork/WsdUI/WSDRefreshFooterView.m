...
- (void)refreshLastUpdatedDate{
	...
	if ([_delegate respondsToSelector:@selector(wsdRefreshTableFooterDataSourceLastUpdated:)]) {
		
		NSDate *date = [_delegate wsdRefreshTableFooterDataSourceLastUpdated:self];
...
- (void)wsdRefreshScrollViewDidScroll:(UIScrollView *)scrollView {
	...
	if (_state == WSDPullRefreshLoading) {
        ...
        //判断是否在加载数据
		BOOL _loading = NO;
		if ([_delegate respondsToSelector:@selector(wsdRefreshTableFooterDataSourceIsLoading:)]) {
			_loading = [_delegate wsdRefreshTableFooterDataSourceIsLoading:self];
		}
...
- (void)wsdRefreshScrollViewDidEndDragging:(UIScrollView *)scrollView {
	...
	if ([self scrollViewOffsetFromBottom:scrollView] <= -65 && !_loading) {
        ...
        //调用委托，加载数据源
		if ([_delegate respondsToSelector:@selector(wsdRefreshTableFooterDidTriggerRefresh:)]) {
			[_delegate wsdRefreshTableFooterDidTriggerRefresh:self];
		}
	}
}

//刷新完数据后调用
- (void)wsdRefreshScrollViewDataSourceDidFinishedLoading:(UIScrollView *)scrollView {
	//完成加载后恢复正常
	[UIView beginAnimations:nil context:NULL];
