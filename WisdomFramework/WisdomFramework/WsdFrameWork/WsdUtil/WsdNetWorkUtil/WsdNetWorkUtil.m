...
+ (ASIHTTPRequest *)genHttpRequestBySoapMessage:(NSString *)soapMessage AndServiceUrl:(NSString *)urlStr{
    ...
    [request addRequestHeader:@"Content-Length" value:[NSString stringWithFormat:@"%lu",(unsigned long)soapMessage.length]];
...
+ (NSDictionary *)genWsdServiceDictionaryByResponseSoapMessage:(NSString *)responseString{
    ...
    if (array.count>1) {
        ...
        dic=[responseJson objectFromJSONStringWithParseOptions:JKParseOptionValidFlags error:&error];
    }
    return dic;
}
